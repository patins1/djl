...
/**
 * A LinearCollection block applies \(m\) linear transformations \(Y_i = X_i W_i + b_i\) for \(i \in
 * [1..m]\) and \(m = \prod_{i=1}^t s_i\). t is typically 1, so compared to a Linear block, the
 * involved shapes have typically one split dimension added which separates the different linear
 * transformations from each other. Another difference to a Linear block is that the weight is not
 * transposed (to align with a different algebraic operation {@link NDArray#matMul(NDArray)}
 * internally). There is currently only a single batch dimension \(x_1\) supported.
 *
...
public class LinearCollection extends AbstractBlock {
    ...
    private int[] shiftBatchDimension;

    private int[] reverseShiftBatchDimension;
    ...
    public NDList linear(NDArray input, NDArray weight, NDArray bias) {
        if (shiftBatchDimension == null) {
            // as the batch dimension is the first dimension in the shape of the input resp. output
            // arrays, it needs to be shifted in order to bring the split axes in front resp. back
            // again to be suitable for matMul;
            // in case there is only one split axes, the transpose array (1,0,2) could be used for
            // both shifts, but for the general case we calculate the transpose arrays here
            int dim = input.getShape().dimension();
            shiftBatchDimension = new int[dim];
            reverseShiftBatchDimension = new int[dim];
            for (int d = 0; d < dim - 2; d++) {
                shiftBatchDimension[d] = d + 1;
                reverseShiftBatchDimension[d + 1] = d;
            }
            shiftBatchDimension[dim - 1] = dim - 1;
            reverseShiftBatchDimension[dim - 1] = dim - 1;
            shiftBatchDimension[dim - 2] = 0;
            reverseShiftBatchDimension[0] = dim - 2;
        }
        NDArray resultArr =
                input.transpose(shiftBatchDimension)
                        .matMul(weight)
                        .transpose(reverseShiftBatchDimension);
        if (bias != null) {
            resultArr.addi(bias);
        }
